...
Define how the Liquid Paradox VS Code extension delivers colorization and editor behavior for `.liquid` files. This capability covers the language registration, language configuration, base HTML + Liquid TextMate grammar, the Paradox-tags injection grammar, packaging, and the contract preserved with the bundled language server.
